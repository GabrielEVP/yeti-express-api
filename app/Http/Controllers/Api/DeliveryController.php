...
class DeliveryController extends Controller
{
    ...
    public function index(): JsonResponse
    {
        ...
        $deliveries = $user->deliveries()->with($this->relations)->get();
    ...
    public function storeClientPayment(
        DeliveryClientPaymentRequest $request,
        Delivery $delivery
    ): JsonResponse {
        $this->authorizeOwner($delivery);
    ...
    private function syncRelatedReceipt(
        Delivery $delivery,
        ?array $receipt
    ): void {
        if ($receipt === null) {
        ...
        if ($delivery->receipt) {
            $delivery
                ->receipt()
                ->update(
                    array_merge($receipt, ["delivery_id" => $delivery->id])
                );
        } else {
            $delivery
                ->receipt()
    ...
    private function logDeliveryEvent(
        string $event,
        Delivery $delivery,
        ?string $section = "deliveries",
    ...
    private function logClientEvent(
        string $event,
        Delivery $delivery,
    ...
    private function logCourierEvent(
        string $event,
        Delivery $delivery,
        ?string $section = "couriers",
        ?string $referenceTable = null,
        ?int $referenceId = null
    ): void {
        CourierEvent::create([
