...
class ClientController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $query = Client::with($this->relations)
            ->when($search, fn($q) => $q->where('legal_name', 'LIKE', "%{$search}%"))
    ...
    {
        $client = Client::create($request->merge(['user_id' => Auth::id()])->all());
    ...
    {
        $this->syncGenericRelation($client, 'addresses', fn($q) => $q->doesntHave('deliveries'));
    ...
    private function syncGenericRelation(Client $client, string $relation, callable $deleteConstraint = null): void
    {
        ...
        foreach ($data as $item) {
            if (!empty($item['id']) && $record = $client->{$relation}()->find($item['id'])) {
                $record->update($item);
            } else {
